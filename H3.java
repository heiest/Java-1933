...
// As a reference for later, this program uses various input and output 
// features that could be handy later on, but are not key to this example. 
// H3.java uses the Scanner class for input, command line input, and mix of 
// Java and C IO.
...
public class H3 {
	...
	if (args.length == 0)  
          ...
          if (args.length == 1)  // next line uses C printf
            ...
            /*  Alternative to above for loop follows
                The following version is more detailed, but allows more options
